...
if(php_sapi_name() === 'cli' && !file_exists('docroot/wp-config.php') && file_exists('docroot/wp-config-sample.php')) {
  $chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789!"#$%&()*+,-./:;<=>?@[]^_`{|}~';
  
  $sample = file('docroot/wp-config-sample.php');
  
  foreach($sample as $key => $line) {
    if(strpos($line, '$table_prefix') !== false) {
      $sample[$key] = '$table_prefix = \''.exec("grep 'DROP TABLE IF EXISTS' sqldump.sql | cut -d _ -f 1 | cut -d '`' -f 2").'_\';'.PHP_EOL;
    }
    
    if(strpos($line, 'Füge hier deine Zeichenkette ein') !== false) {
      $sample[$key] = str_replace('Füge hier deine Zeichenkette ein', substr(str_shuffle($chars), 0, 64), $line);
    }
  }
  
  unset($sample[0]);
  array_unshift($sample, 'define(\'WP_SITEURL\', \'http://localhost:8000\');'.PHP_EOL);
  array_unshift($sample, 'define(\'WP_HOME\', \'http://localhost:8000\');'.PHP_EOL);
  array_unshift($sample, '<?php'.PHP_EOL);
  
  file_put_contents('docroot/wp-config.php', $sample);
}

foreach(file(php_sapi_name() === 'cli' ? 'docroot/wp-config.php' : 'wp-config.php') as $line) {
  if(strpos($line, 'DB_HOST') !== false) eval($line);
  if(strpos($line, 'DB_USER') !== false) eval($line);
  if(strpos($line, 'DB_PASSWORD') !== false) eval($line);
  if(strpos($line, 'DB_NAME') !== false) eval($line);
}

if(php_sapi_name() === 'cli') {
  shell_exec('echo CREATE DATABASE IF NOT EXISTS '.DB_NAME.' | /usr/local/mysql/bin/mysql -S /tmp/<repo-name>.mysql.sock');
  shell_exec('/usr/local/mysql/bin/mysql -S /tmp/<repo-name>.mysql.sock '.DB_NAME.' < sqldump.sql');
} else {
